...
if tab_choice == "🔁 Rotor Tracker":
    ...
    with tabs[2]:
        ...
        chat_query = st.text_input("💬 Ask about rotors:", placeholder="e.g., history 1803 | pending | coming rotors | buyers list")
        if not chat_query:
        ...
        elif "all buyers" in query or "buyers list" in query: movement = "buyers"
        ...
        if target_size:
            ...
            hist["Value (₹)"] = hist.apply(lambda r: f"₹{calc_value(r['Size (mm)'],r['Quantity']):,.0f}", axis=1)
            disp = hist.copy()
            disp["Date"] = disp["Date"].dt.strftime("%Y-%m-%d")
            disp["Pending"] = disp["Pending"].apply(lambda x: "Yes" if x else "No")
            st.dataframe(disp[["Date","Type","Quantity","Remarks","Status","Pending","Value (₹)"]].rename(columns={"Remarks":"Buyer/Supplier"}), use_container_width=True, hide_index=True)
            st.stop()

        # ─ All buyers ─
        if movement == "buyers":
            st.subheader("👥 All Buyers")
            bout = df_cb[df_cb["Type"]=="Outgoing"].groupby("Remarks").agg({"Quantity":"sum","Date":["min","max","count"]}).reset_index()
            bout.columns = ["Buyer","Total Qty","First","Last","Transactions"]
            bout["Value (₹)"] = bout.apply(lambda r: f"₹{get_price(0)*0:,.0f}", axis=1)
        ...
        if movement == "coming":
            ...
            if com.empty:
                st.info("No future rotors.")
                st.stop()
            com["Value (₹)"] = com.apply(lambda r: f"₹{calc_value(r['Size (mm)'],r['Quantity']):,.0f}", axis=1)
            c1,c2 = st.columns(2)
            c1.metric("Total Coming", int(com["Quantity"].sum()))
            c2.metric("Total Value", f"₹{sum([calc_value(r['Size (mm)'],r['Quantity']) for _,r in com.iterrows()]):,.0f}")
            disp = com.copy()
            disp["Date"] = disp["Date"].dt.strftime("%Y-%m-%d")
            st.dataframe(disp[["Date","Size (mm)","Quantity","Remarks","Value (₹)"]].rename(columns={"Remarks":"Supplier","Size (mm)":"Size"}), use_container_width=True, hide_index=True)
            st.stop()
        ...
        if movement == "pending":
            filtered = filtered[(filtered["Type"]=="Outgoing")&(filtered["Pending"]==True)]
        elif movement == "incoming":
            filtered = filtered[filtered["Type"]=="Inward"]
        elif movement == "outgoing":
            filtered = filtered[filtered["Type"]=="Outgoing"]
        if month_num:
            ...
            end   = (datetime(year_num, month_num+1, 1) - timedelta(days=1)) if month_num < 12 else datetime(year_num,12,31)
            filtered = filtered[(filtered["Date"]>=start)&(filtered["Date"]<=end)]
        filtered = filtered.dropna(subset=["Size (mm)","Quantity"])

        if filtered.empty:
            st.warning(f"No records found for: '{chat_query}'")
            st.stop()

        filtered["Value (₹)"] = filtered.apply(lambda r: calc_value(r["Size (mm)"],r["Quantity"]), axis=1)
        total_qty = int(filtered["Quantity"].sum())
        total_val = filtered["Value (₹)"].sum()
        c1,c2,c3 = st.columns(3)
        c1.metric("Total Rotors", f"{total_qty:,}")
        c2.metric("Total Value", f"₹{total_val:,.0f}")
        c3.metric("Avg Size", f"{filtered['Size (mm)'].mean():.0f}mm")

        disp2 = filtered.copy()
        disp2["Date"] = disp2["Date"].dt.strftime("%Y-%m-%d")
        disp2["Value (₹)"] = disp2["Value (₹)"].apply(lambda x: f"₹{x:,.0f}")
        disp2["Pending"] = disp2["Pending"].apply(lambda x: "Yes" if x else "No")
        grp = disp2.groupby(["Remarks","Size (mm)"]).agg({"Quantity":"sum"}).reset_index()
        st.dataframe(grp.rename(columns={"Remarks":"Buyer","Size (mm)":"Size","Quantity":"Total Qty"}), use_container_width=True, hide_index=True)
